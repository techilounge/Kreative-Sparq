...
- **Truthful empty space:** When proof or publications are unavailable, use a clearly labelled, ruled status field and approved state copy. Empty space may carry hierarchy. Do not fill it with sample cards, mock results, fictional imagery, placeholder people, or unpublished metadata.
- **Publication-state distinction:** The Insights pre-publication state applies only while zero articles are approved. It uses the `Publication status` eyebrow and review-in-progress copy, removes the hero action, publishes no category controls, and emits `noindex,follow`. The filter-empty state is reserved for a future populated index after article cards and working category filters exist. Its “No article matches that filter yet” copy and “View all insights” action must never appear as the zero-publication state.
- **Page-specific composition:** Work uses a pending archive and results-integrity band. About uses a dark point-of-view chapter, ruled belief grid, and operating-principles list. Insights uses a pre-publication status index while no articles are approved. Shared tokens and controls remain stable while the chapter structures differ.
...
- **Actions:** Public destinations may use the locked primary button or editorial text link. Contact, booking, and project links now reach truthful Phase 8 availability pages and must not imply successful booking, contact, or submission. Insights has no hero action while zero articles are public; do not add a control that only scrolls to its pre-publication status.
...
## 12. Proven conversion and legal-route fallback patterns

These rules apply while the external details required for a working submission, calendar, or final legal policy remain unapproved.

- **Availability panel:** Use one consistent ruled status panel with a small eyebrow, direct heading, concise reason, and a short list of unresolved requirements. It must state the present condition without promising a response, delivery, storage, or future date. The panel may appear in different positions according to each route’s reading order.
- **Disabled capability:** When no monitored destination exists, render no form, input, select, textarea, submit control, or server action. When no approved event URL exists, render no calendar iframe or simulated time slots. A visible status explanation is the complete fallback experience; controls must not accept data that cannot be handled.
- **Project-brief preview:** Approved future questions may be shown as numbered, read-only editorial guidance. Label the chapter as an outline, keep answer fields absent, and pair it with the availability and privacy status so it cannot be mistaken for a working submission flow.
- **Safe thank-you state:** Direct access to `/thank-you` says that no submission was recorded. Do not infer success from query strings, reflect contact data, expose identifiers, or show response-time promises. A genuine success state may be added only when a verified server result can identify the completed operation safely.
- **Legal review state:** Privacy and Terms may publish a clearly labelled review status and a factual description of current site operation. Do not publish draft clauses, placeholder entity/contact fields, guessed data providers, retention periods, rights procedures, liability limits, governing law, or policy dates. Final policy content requires confirmed facts and legal review.
- **Indexing:** Contact, project brief, booking, thank-you, and review-state legal pages use `noindex,follow` until their content and function are approved for release. Canonical and truthful Open Graph metadata remain present. BreadcrumbList data is allowed only when the matching visible breadcrumb is rendered; conversion routes do not imply Organization, LocalBusiness, Person, Review, or service-result facts.
- **Route-specific composition:** Contact pairs availability with service-area and reassurance content. Start a project uses a numbered brief outline and privacy band. Booking uses call-purpose and preparation chapters. Thank you is deliberately short. Privacy and Terms use compact status indexes. Shared tokens and status primitives must not flatten these routes into one template.
- **Responsive behavior:** Collapse all route splits below 768 px, preserve 44 px navigation and action targets, keep status copy within the body measure, and use ruled rows for long planning lists. The four-group footer stays below the brand statement through 1279 px to prevent intermediate-width overflow; its full desktop grid begins at 1280 px.
